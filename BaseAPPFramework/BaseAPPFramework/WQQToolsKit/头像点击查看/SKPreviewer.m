...
- (void)previewFromImageView:(UIImageView *)imageView container:(UIView *)container
{
    ...
    self.containerView.jt_origin = CGPointZero;
    self.containerView.jt_width = self.jt_width;
    
    UIImage *image = self.fromTheImageView.image;
    
    if (image.size.height / image.size.width > self.jt_height / self.jt_width) {
        self.containerView.jt_height = floor(image.size.height / image.size.width / self.jt_width);
    }else {
        CGFloat height = image.size.height / image.size.width * self.jt_width;
        if (height < 1.0 || isnan(height)) {
            height = self.jt_height;
        }
        height = floor(height);
        self.containerView.jt_height = height;
        self.containerView.jt_centerY = self.jt_height / 2;
    }
    
    if (self.containerView.jt_height > self.jt_height && self.containerView.jt_height - self.jt_height <= 1) {
        self.containerView.jt_height = self.jt_height;
    }
    
    self.scrollView.contentSize = CGSizeMake(self.jt_height, MAX(self.containerView.jt_height,self.jt_height));
    [self.scrollView scrollRectToVisible:self.bounds animated:NO];
    
    if (self.containerView.jt_height <= self.jt_height) {
...
//    放大点击区域
    if (self.scrollView.zoomScale > 1.0) {
        ...
        CGFloat x = self.jt_width / newZoomScale;
        CGFloat y = self.jt_height / newZoomScale;
